...
# ══════════════════════════════════════════════════════════════
# ADMIN DASHBOARD + ACCESS LOGGING (embedded, no separate file)
# ══════════════════════════════════════════════════════════════
import sqlite3 as _sqlite3_admin
import time as _time_admin

_ADMIN_SECRET = os.environ.get("ADMIN_SECRET", "kevin2025admin")
_LOG_DB = os.path.join(os.path.dirname(os.path.abspath(__file__)), "logs", "access.db")
_log_lock = threading.Lock()

def _init_log_db():
    os.makedirs(os.path.dirname(_LOG_DB), exist_ok=True)
    conn = _sqlite3_admin.connect(_LOG_DB)
    conn.execute("""CREATE TABLE IF NOT EXISTS access_log (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        ts TEXT, ts_unix REAL, ip TEXT, country TEXT,
        method TEXT, path TEXT, query TEXT, status INTEGER,
        resp_ms INTEGER, browser TEXT, device TEXT, os TEXT,
        referrer TEXT, is_bot INTEGER DEFAULT 0
    )""")
    conn.execute("""CREATE TABLE IF NOT EXISTS search_log (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        ts TEXT, ts_unix REAL, ip TEXT, query TEXT,
        results_ct INTEGER, result_syms TEXT
    )""")
    conn.execute("""CREATE TABLE IF NOT EXISTS error_log (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        ts TEXT, ts_unix REAL, ip TEXT,
        path TEXT, status INTEGER, error_msg TEXT
    )""")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_al_ts ON access_log(ts_unix)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_sl_q  ON search_log(query)")
    conn.commit(); conn.close()
...
def _parse_ua(ua):
    u = (ua or "").lower()
    bots    = ["bot","crawler","spider","python","curl","wget","postman","go-http","java/"]
    is_bot  = any(b in u for b in bots)
    browser = ("Edge" if "edg/" in u else "Chrome" if "chrome/" in u else
               "Firefox" if "firefox" in u else "Safari" if "safari" in u and "chrome" not in u else "Other")
    os_name = ("Windows" if "windows" in u else "macOS" if "mac os" in u else
...
def _write_access_log(ip, path, query, status, resp_ms, ua, referrer):
    def _w():
        if not _LOG_DB_OK: return
        import datetime as _dt
        ts  = _dt.datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")
        br, dv, os_n, bot = _parse_ua(ua)
        try:
            with _log_lock:
                conn = _sqlite3_admin.connect(_LOG_DB)
                conn.execute(
                    "INSERT INTO access_log (ts,ts_unix,ip,method,path,query,status,resp_ms,browser,device,os,referrer,is_bot) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",
                    (ts, _time_admin.time(), ip, "GET", path, (query or "")[:200],
                     status, resp_ms, br, dv, os_n, (referrer or "")[:200], int(bot))
                )
                conn.execute("DELETE FROM access_log WHERE id NOT IN (SELECT id FROM access_log ORDER BY id DESC LIMIT 50000)")
                conn.commit(); conn.close()
        except Exception: pass
    threading.Thread(target=_w, daemon=True).start()

def _write_search_log(ip, query, count, syms):
    def _w():
        if not _LOG_DB_OK: return
        import datetime as _dt
        ts = _dt.datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")
        try:
            with _log_lock:
                conn = _sqlite3_admin.connect(_LOG_DB)
                conn.execute("INSERT INTO search_log (ts,ts_unix,ip,query,results_ct,result_syms) VALUES (?,?,?,?,?,?)",
                             (ts, _time_admin.time(), ip, query, count, json.dumps(syms[:10])))
                conn.execute("DELETE FROM search_log WHERE id NOT IN (SELECT id FROM search_log ORDER BY id DESC LIMIT 20000)")
                conn.commit(); conn.close()
        except Exception: pass
    threading.Thread(target=_w, daemon=True).start()

def _write_error_log(ip, path, status, msg):
    def _w():
        if not _LOG_DB_OK: return
        import datetime as _dt
        ts = _dt.datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")
        try:
            with _log_lock:
                conn = _sqlite3_admin.connect(_LOG_DB)
                conn.execute("INSERT INTO error_log (ts,ts_unix,ip,path,status,error_msg) VALUES (?,?,?,?,?,?)",
                             (ts, _time_admin.time(), ip, path, status, str(msg)[:400]))
                conn.commit(); conn.close()
        except Exception: pass
    threading.Thread(target=_w, daemon=True).start()
...
def _after_req(response):
    try:
        from flask import g
        path = freq.path
        if any(path.startswith(s) for s in ["/static", "/favicon"]):
            return response
        ms  = int((_time_admin.time() - getattr(g, "_req_start", _time_admin.time())) * 1000)
        ip  = (freq.headers.get("X-Forwarded-For") or freq.remote_addr or "").split(",")[0].strip()
        ua  = freq.headers.get("User-Agent", "")
        ref = freq.headers.get("Referer", "")
        qs  = freq.query_string.decode("utf-8", errors="replace")[:200]
        _write_access_log(ip, path, qs, response.status_code, ms, ua, ref)
        if response.status_code >= 400:
            _write_error_log(ip, path, response.status_code, f"HTTP {response.status_code}")
    except Exception:
        pass
    return response

def _admin_check():
    s = freq.args.get("secret") or freq.headers.get("X-Admin-Secret","")
    return s == _ADMIN_SECRET

@app.route("/api/admin/logs")
def admin_logs():
    if not _admin_check():
        return jsonify({"error":"Unauthorized — add ?secret=YOUR_ADMIN_SECRET"}), 401
    log_type  = freq.args.get("type","access")
    limit     = min(int(freq.args.get("limit",200) or 200), 2000)
    offset    = int(freq.args.get("offset",0) or 0)
    ip_filter = freq.args.get("ip","")
    dt_filter = freq.args.get("date","")
    conn = _sqlite3_admin.connect(_LOG_DB)
    conn.row_factory = _sqlite3_admin.Row
    if log_type == "search":
        q = "SELECT * FROM search_log"; params = []
        if ip_filter: q += " WHERE ip=?"; params.append(ip_filter)
        if dt_filter:
            q += (" AND " if ip_filter else " WHERE ") + "ts LIKE ?"; params.append(dt_filter+"%")
        ...
        rows  = [dict(r) for r in conn.execute("SELECT * FROM error_log ORDER BY id DESC LIMIT ? OFFSET ?", [limit,offset]).fetchall()]
        total = conn.execute("SELECT COUNT(*) FROM error_log").fetchone()[0]
    else:
        q = "SELECT id,ts,ip,method,path,query,status,resp_ms,browser,device,os,referrer,is_bot FROM access_log"
        params = []
        where = []
        if ip_filter: where.append(("ip=?", ip_filter))
        if dt_filter: where.append(("ts LIKE ?", dt_filter+"%"))
        if where:
    ...
    conn.close()
    return jsonify({"success":True,"type":log_type,"total":total,"limit":limit,"offset":offset,"rows":rows})
...
def admin_stats():
    ...
    def q(sql, p=()): return conn.execute(sql, p).fetchone()
    total       = q("SELECT COUNT(*) FROM access_log")[0]
    today_hits  = q("SELECT COUNT(*) FROM access_log WHERE ts LIKE ?", (today+"%",))[0]
    uniq_all    = q("SELECT COUNT(DISTINCT ip) FROM access_log")[0]
    uniq_today  = q("SELECT COUNT(DISTINCT ip) FROM access_log WHERE ts LIKE ?", (today+"%",))[0]
    searches    = q("SELECT COUNT(*) FROM search_log")[0]
    errors      = q("SELECT COUNT(*) FROM error_log")[0]
    avg_ms      = q("SELECT AVG(resp_ms) FROM access_log WHERE ts LIKE ?", (today+"%",))[0]
    top_paths   = conn.execute("SELECT path,COUNT(*) h FROM access_log WHERE ts LIKE ? GROUP BY path ORDER BY h DESC LIMIT 10",(today+"%",)).fetchall()
    top_ips     = conn.execute("SELECT ip,COUNT(*) h FROM access_log WHERE ts LIKE ? GROUP BY ip ORDER BY h DESC LIMIT 15",(today+"%",)).fetchall()
    top_searches= conn.execute("SELECT query,COUNT(*) c FROM search_log GROUP BY query ORDER BY c DESC LIMIT 20").fetchall()
    hourly      = conn.execute("SELECT substr(ts,12,2) hr,COUNT(*) h FROM access_log WHERE ts LIKE ? GROUP BY hr ORDER BY hr",(today+"%",)).fetchall()
    countries   = conn.execute("SELECT country,COUNT(*) h FROM access_log WHERE ts LIKE ? GROUP BY country ORDER BY h DESC LIMIT 10",(today+"%",)).fetchall()
    devices     = conn.execute("SELECT device,COUNT(*) h FROM access_log WHERE ts LIKE ? GROUP BY device ORDER BY h DESC",(today+"%",)).fetchall()
    browsers    = conn.execute("SELECT browser,COUNT(*) h FROM access_log WHERE ts LIKE ? GROUP BY browser ORDER BY h DESC",(today+"%",)).fetchall()
    recent_err  = conn.execute("SELECT ts,ip,path,status,error_msg FROM error_log ORDER BY id DESC LIMIT 20").fetchall()
    conn.close()
    return jsonify({"success":True,
        "overview":{"total_hits_ever":total,"hits_today":today_hits,"unique_ips_ever":uniq_all,
                    "unique_ips_today":uniq_today,"total_searches":searches,"total_errors":errors,
                    "avg_response_ms_today":round(avg_ms or 0,1)},
        "top_paths_today":  [{"path":r[0],"hits":r[1]} for r in top_paths],
        "top_ips_today":    [{"ip":r[0],"hits":r[1]} for r in top_ips],
        ...
        "recent_errors":    [{"ts":r[0],"ip":r[1],"path":r[2],"status":r[3],"msg":r[4]} for r in recent_err],
    })

@app.route("/admin")
@app.route("/admin/dashboard")
def admin_dashboard():
    secret = freq.args.get("secret","")
    if secret != _ADMIN_SECRET:
        return """<!DOCTYPE html><html><head><title>Admin</title>
<style>*{box-sizing:border-box;margin:0;padding:0}body{background:#060d1a;display:flex;align-items:center;justify-content:center;height:100vh;font-family:system-ui}
.box{background:#0c1628;border:1px solid rgba(255,255,255,.1);border-radius:20px;padding:40px;width:340px;text-align:center}
h2{color:#38bdf8;margin-bottom:20px;font-size:20px}
input{width:100%;padding:11px 14px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.12);border-radius:10px;color:#e8f0ff;font-size:14px;margin-bottom:14px;outline:none}
button{width:100%;padding:11px;background:linear-gradient(135deg,#38bdf8,#2dd4bf);border:none;border-radius:10px;font-size:14px;font-weight:700;cursor:pointer;color:#060d1a}
</style></head><body><div class="box"><h2>&#128274; Admin Access</h2>
<form onsubmit="go(event)"><input type="password" id="s" placeholder="Enter admin secret" autofocus>
<button>Enter</button></form></div>
<script>function go(e){e.preventDefault();window.location='/admin/dashboard?secret='+document.getElementById('s').value}</script>
</body></html>"""
    return f"""<!DOCTYPE html><html><head><meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>Admin — Stock Intelligence Pro</title>
<style>
*{{box-sizing:border-box;margin:0;padding:0}}
body{{background:#060d1a;color:#e8f0ff;font-family:'Segoe UI',system-ui,sans-serif;min-height:100vh;font-size:14px}}
.hdr{{background:#0c1628;border-bottom:1px solid rgba(255,255,255,.08);padding:14px 24px;display:flex;align-items:center;justify-content:space-between;position:sticky;top:0;z-index:10}}
.hdr h1{{font-size:16px;font-weight:700;color:#38bdf8}}
...
.card{{background:#0c1628;border:1px solid rgba(255,255,255,.07);border-radius:12px;padding:16px}}
.sv{{font-size:26px;font-weight:700;font-family:monospace;color:#38bdf8;margin:5px 0}}
.sl{{font-size:10px;color:#5a7090;font-weight:700;text-transform:uppercase;letter-spacing:.08em}}
.st{{font-size:12px;font-weight:700;color:#38bdf8;margin-bottom:10px;text-transform:uppercase;letter-spacing:.08em}}
table{{width:100%;border-collapse:collapse;font-size:12px}}
th{{color:#5a7090;font-weight:600;text-transform:uppercase;letter-spacing:.06em;padding:5px 8px;border-bottom:1px solid rgba(255,255,255,.06);text-align:left}}
td{{padding:6px 8px;border-bottom:1px solid rgba(255,255,255,.04);color:#b8c8e8}}
tr:hover td{{background:rgba(255,255,255,.02)}}
.ok{{background:rgba(16,185,129,.12);color:#10b981;padding:2px 7px;border-radius:99px;font-size:10px;font-weight:700}}
.er{{background:rgba(248,113,113,.12);color:#f87171;padding:2px 7px;border-radius:99px;font-size:10px;font-weight:700}}
.btn{{background:rgba(56,189,248,.1);border:1px solid rgba(56,189,248,.2);color:#38bdf8;padding:6px 14px;border-radius:7px;font-size:12px;cursor:pointer;font-weight:600}}
.btn:hover{{background:rgba(56,189,248,.2)}}
.tabs{{display:flex;gap:5px;margin-bottom:12px;flex-wrap:wrap}}
.tab{{padding:6px 14px;border-radius:7px;font-size:12px;font-weight:600;cursor:pointer;border:1px solid rgba(255,255,255,.07);background:transparent;color:#5a7090;transition:all .15s}}
.tab.a,.tab:hover{{background:rgba(56,189,248,.1);color:#38bdf8;border-color:rgba(56,189,248,.25)}}
input.fi{{background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.1);border-radius:7px;padding:6px 10px;color:#e8f0ff;font-size:12px;width:160px;outline:none}}
@media(max-width:900px){{.g4{{grid-template-columns:1fr 1fr}}.g3{{grid-template-columns:1fr}}}}
</style></head><body>
<div class="hdr">
  <h1>&#128202; Stock Intelligence Pro — Admin</h1>
  <div style="display:flex;gap:8px;align-items:center">
    <span id="clk" style="font-size:11px;color:#5a7090"></span>
...
<div class="shell">
  ...
  <div class="card" style="margin-top:16px">
    <div class="st">Raw Log Viewer</div>
    <div class="tabs">
      <button class="tab a" onclick="loadLogs('access',this)">Access Logs</button>
    ...
    <div style="display:flex;gap:8px;margin-bottom:10px;flex-wrap:wrap">
      <input class="fi" id="ipf" placeholder="Filter by IP…">
      <input class="fi" id="dtf" type="date">
      <button class="btn" onclick="applyFilter()">Filter</button>
      <button class="btn" onclick="exportCSV()">&#11015; CSV</button>
    </div>
    <div id="lt" style="overflow-x:auto;max-height:450px;overflow-y:auto"></div>
  </div>
</div>
<script>
const S='{secret}';let ct='access',cd=[];
async function loadStats(){{
  const r=await fetch('/api/admin/stats?secret='+S),d=await r.json();
  if(!d.success)return;const o=d.overview;
  document.getElementById('sc').innerHTML=`
  <div class="g4">
    <div class="card"><div class="sl">Total Hits Ever</div><div class="sv">${{o.total_hits_ever.toLocaleString()}}</div></div>
    <div class="card"><div class="sl">Hits Today</div><div class="sv" style="color:#2dd4bf">${{o.hits_today.toLocaleString()}}</div></div>
    <div class="card"><div class="sl">Unique IPs Today</div><div class="sv" style="color:#a78bfa">${{o.unique_ips_today.toLocaleString()}}</div></div>
    <div class="card"><div class="sl">Avg Response ms</div><div class="sv" style="color:#fbbf24">${{o.avg_response_ms_today}}</div></div>
  </div>
  <div class="g4">
    <div class="card"><div class="sl">Total Searches</div><div class="sv">${{o.total_searches.toLocaleString()}}</div></div>
    <div class="card"><div class="sl">Total Errors</div><div class="sv" style="color:#f87171">${{o.total_errors.toLocaleString()}}</div></div>
    <div class="card"><div class="sl">Unique IPs Ever</div><div class="sv">${{o.unique_ips_ever.toLocaleString()}}</div></div>
    <div class="card"><div class="sl">DB Status</div><div class="sv" style="color:#10b981;font-size:18px">LIVE</div></div>
  </div>
  <div class="g3">
    <div class="card"><div class="st">Top Pages Today</div><table><thead><tr><th>Path</th><th>Hits</th></tr></thead><tbody>
    ${{d.top_paths_today.map(r=>`<tr><td style="font-family:monospace;color:#38bdf8">${{r.path}}</td><td>${{r.hits}}</td></tr>`).join('')}}</tbody></table></div>
    <div class="card"><div class="st">Top Searches</div><table><thead><tr><th>Query</th><th>#</th></tr></thead><tbody>
    ${{d.top_searches.map(r=>`<tr><td style="color:#fbbf24">${{r.query}}</td><td>${{r.count}}</td></tr>`).join('')}}</tbody></table></div>
    <div class="card"><div class="st">Top IPs Today</div><table><thead><tr><th>IP</th><th>Hits</th></tr></thead><tbody>
    ${{d.top_ips_today.map(r=>`<tr><td style="font-family:monospace">${{r.ip}}</td><td style="color:#38bdf8;font-weight:700">${{r.hits}}</td></tr>`).join('')}}</tbody></table></div>
  </div>
  <div class="g2">
    <div class="card"><div class="st">Devices Today</div>
    ${{d.devices_today.map(r=>`<div style="display:flex;justify-content:space-between;margin-bottom:7px;font-size:12px"><span>${{r.device}}</span><span style="color:#2dd4bf;font-weight:700">${{r.hits}}</span></div>`).join('')}}<br>
    <div class="st">Browsers</div>
    ${{d.browsers_today.map(r=>`<div style="display:flex;justify-content:space-between;margin-bottom:7px;font-size:12px"><span>${{r.browser}}</span><span style="color:#a78bfa;font-weight:700">${{r.hits}}</span></div>`).join('')}}</div>
    <div class="card"><div class="st">Recent Errors</div><table><thead><tr><th>Time</th><th>IP</th><th>Path</th><th>Status</th></tr></thead><tbody>
    ${{d.recent_errors.map(r=>`<tr><td style="color:#5a7090;white-space:nowrap">${{r.ts}}</td><td style="font-family:monospace">${{r.ip||'?'}}</td><td style="color:#38bdf8;font-family:monospace">${{r.path}}</td><td><span class="${{r.status>=500?'er':'ok'}}">${{r.status}}</span></td></tr>`).join('')}}</tbody></table></div>
  </div>`;
}}
...
async function applyFilter(){{
  const ip=document.getElementById('ipf').value,dt=document.getElementById('dtf').value;
  let url=`/api/admin/logs?secret=${{S}}&type=${{ct}}&limit=300`;
  if(ip)url+=`&ip=${{ip}}`;if(dt)url+=`&date=${{dt}}`;
  const d=await(await fetch(url)).json();
  cd=d.rows||[];renderTable(cd,ct);
}}
function renderTable(rows,type){{
  let h='';
  if(type==='access'){{
    h=`<table><thead><tr><th>#</th><th>Time</th><th>IP</th><th>Path</th><th>Status</th><th>ms</th><th>Browser</th><th>Device</th></tr></thead><tbody>
    ${{rows.map(r=>`<tr><td style="color:#5a7090">${{r.id}}</td><td style="white-space:nowrap;color:#5a7090">${{r.ts}}</td>
    ...
    <td><span class="${{r.status>=400?'er':'ok'}}">${{r.status}}</span></td>
    <td style="color:${{r.resp_ms>1000?'#f87171':r.resp_ms>400?'#fbbf24':'#10b981'}}">${{r.resp_ms}}</td>
    <td>${{r.browser||'?'}}</td><td>${{r.device||'?'}}</td></tr>`).join('')}}</tbody></table>`;
  }}else if(type==='search'){{
    h=`<table><thead><tr><th>#</th><th>Time</th><th>IP</th><th>Query</th><th>Results</th></tr></thead><tbody>
    ${{rows.map(r=>`<tr><td style="color:#5a7090">${{r.id}}</td><td style="white-space:nowrap;color:#5a7090">${{r.ts}}</td>
    <td style="font-family:monospace">${{r.ip||'?'}}</td><td style="color:#fbbf24;font-weight:600">${{r.query}}</td><td style="color:#38bdf8">${{r.results_ct}}</td></tr>`).join('')}}</tbody></table>`;
  }}else{{
    h=`<table><thead><tr><th>#</th><th>Time</th><th>IP</th><th>Path</th><th>Status</th><th>Error</th></tr></thead><tbody>
    ${{rows.map(r=>`<tr><td style="color:#5a7090">${{r.id}}</td><td style="white-space:nowrap;color:#5a7090">${{r.ts}}</td>
    <td style="font-family:monospace">${{r.ip||'?'}}</td><td style="color:#38bdf8;font-family:monospace">${{r.path}}</td>
    <td><span class="er">${{r.status}}</span></td><td style="color:#f87171">${{r.error_msg}}</td></tr>`).join('')}}</tbody></table>`;
  }}
  document.getElementById('lt').innerHTML=h;
}}
function exportCSV(){{
  if(!cd.length)return;
  const k=Object.keys(cd[0]);
  const csv=[k.join(','),...cd.map(r=>k.map(k=>JSON.stringify(r[k]??'')).join(','))].join('\n');
  const a=document.createElement('a');a.href='data:text/csv;charset=utf-8,'+encodeURIComponent(csv);
  a.download=`logs_${{ct}}_${{new Date().toISOString().slice(0,10)}}.csv`;a.click();
}}
setInterval(()=>{{document.getElementById('clk').textContent=new Date().toLocaleTimeString('en-IN',{{timeZone:'Asia/Kolkata',hour12:true}})+' IST'}},1000);
loadStats();loadLogs('access',document.querySelector('.tab.a'));
setInterval(loadStats,30000);
</script></body></html>"""


# Also override log_search and log_error in the search route to use our embedded versions
# Update the search route to use _write_search_log and _write_error_log
